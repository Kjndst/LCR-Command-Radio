...
! grep -q 'REPLACE_WITH_A_LONG' "$ENV" || { echo 'FAIL: radio secret still placeholder'; exit 2; }
...
docker compose -f compose.yaml build bot
docker compose -f compose.yaml up -d bot
